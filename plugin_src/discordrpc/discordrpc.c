...
#include "pc/sharedlibs.h"
#include "discordrpc.h"

#include <stdlib.h>
bool configDiscordRPC = true;
extern int options_size;
extern struct ConfigOption *options;

void *segmented_to_virtual(const void *addr);
void add_function(void (*func)());

#define DISCORDLIB "libdiscord-rpc" LIBEXT
...
static void init_discord(void) {
    ...
    Discord_Initialize(DISCORD_APP_ID, &handlers, true, NULL);
...
void init_func()
{
    struct ConfigOption *newoptions;
    size_t size = (sizeof(struct ConfigOption) * ++options_size);

    newoptions = malloc(size);
    memcpy(newoptions, options, size - sizeof(struct ConfigOption));
    free(options);
    options = newoptions;

    options[options_size-1].name = "discordrpc_enabled";
    options[options_size-1].type = CONFIG_TYPE_BOOL;
    options[options_size-1].boolValue = &configDiscordRPC;

    discord_init();

    add_function(discord_update_rich_presence);

    printf("[Discord] Initiated\n");
}
